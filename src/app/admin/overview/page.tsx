...
const Card = ({ children, className = "" }) => (
  <div className={`rounded-xl border bg-card text-card-foreground shadow-xl dark:bg-neutral-900 dark:border-neutral-800 ${className}`}>
    {children}
  </div>
);
const CardHeader = ({ children, className = "" }) => <div className={`flex flex-col space-y-1.5 p-6 ${className}`}>{children}</div>;
const CardTitle = ({ children, className = "" }) => <h3 className={`text-xl font-semibold leading-none tracking-tight text-gray-50 ${className}`}>{children}</h3>;
const CardDescription = ({ children, className = "" }) => <p className={`text-sm text-gray-400 ${className}`}>{children}</p>;
const CardContent = ({ children, className = "" }) => <div className={`p-6 pt-0 ${className}`}>{children}</div>;
const Badge = ({ children, variant, className = "" }) => {
    let baseStyle = "inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2";
    if (variant === "success") {
...
// --- MOCK DATA ---
const mockStats = [
  { label: "Total Revenue", value: "₦2.5M", icon: DollarSign, change: "+12%", color: "text-emerald-400", trend: "up" },
  { label: "Orders Placed", value: "156", icon: ShoppingBag, change: "+8%", color: "text-rose-400", trend: "up" },
  { label: "Products in Stock", value: "42", icon: Package, change: "+3%", color: "text-blue-400", trend: "up" },
  { label: "New Customers", value: "892", icon: Users, change: "-4%", color: "text-red-400", trend: "down" },
];
...
const mockOrders = [
  ...
  { id: "RM-004", customer: "Michael B.", amount: 18900, status: "delivered", date: "2025-10-25" },
  { id: "RM-005", customer: "Jessica F.", amount: 75200, status: "processing", date: "2025-10-26" },
];

const mockTopProducts = [
    { id: 1, name: "Velvet Luxe Lipstick", sales: 85, revenue: 212500, category: "Lipstick" },
    { id: 2, name: "Opulence Eyeshadow", sales: 75, revenue: 262500, category: "Eyeshadow" },
    { id: 3, name: "PerfectSkin Foundation", sales: 60, revenue: 168000, category: "Foundation" },
    { id: 4, name: "HydraGlow Lip Gloss", sales: 45, revenue: 81000, category: "Lip Gloss" },
];
            ...
            <Card className="p-4 sm:p-6 transition-all duration-300 hover:border-rose-600/50">
                <div className="flex items-start justify-between">
                    <div>
                        ...
                        <h3 className="text-3xl font-extrabold mt-1 text-gray-50 tracking-tight">{stat.value}</h3>
                    </div>
                    <div className={`p-3 rounded-full ${trendBg} ${stat.color} shadow-lg`}>
                    ...
                    {mockTopProducts.slice(0, 4).map((p, i) => (
                        ...
                        >
                            ...
                            <div className="text-right">
                                <p className="text-sm font-semibold text-gray-50">₦{(p.revenue / 1000).toFixed(1)}K</p>
                        ...
                        <tbody>
                            {mockOrders.map((o, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        <td className="py-3 px-6 font-semibold text-gray-100">{o.id}</td>
                                        <td className="py-3 px-6 text-gray-300">{o.customer}</td>
                                        <td className="py-3 px-6 text-gray-300">₦{o.amount.toLocaleString()}</td>
                                        <td className="py-3 px-6">
                                            <Badge variant={status.variant}>
                                                <Clock className="w-3 h-3 mr-1" /> {status.text}
                                            </Badge>
                                        </td>
                                        <td className="py-3 px-6 text-gray-400">{new Date(o.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</td>
                                    </motion.tr>
                                );
                            })}
                        </tbody>
